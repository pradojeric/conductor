...
import 'package:flutter_app/shared_service_login.dart';
...
class ConductorBloc {
  ...
  Future<RideModel> getRideToday() async {
    final prefs = await SharedPreferences.getInstance();
    String token = jsonDecode(prefs.getString("login_details"))["token"];
    String rideCode = prefs.getString('ride_code') != '' ? '?ride_code=${prefs.getString('ride_code')}' : '';
  ...
  Future<void> rideDeparture(dynamic ride) async {
    ...
    final response = await API.postRequest(uri: url, token: token, requestModel: ride);
  ...
  Future<RideModel> rideArrival() async {
    ...
    final response = await API.postRequest(uri: url, token: token, requestModel: body);
  ...
  Future issueTicket(String bookingCode) async {
    ...
    String url = '$API_URL/api/issue-receipt/$bookingCode';
